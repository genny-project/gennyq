...
KEYCLOAK_RESPONSE=`curl -s -X POST https://keycloak-bali.gada.io/auth/realms/internmatch/protocol/openid-connect/token  -H "Content-Type: application/x-www-form-urlencoded" -d 'username=user1' -d "password=${password}" -d 'grant_type=password' -d 'client_id=alyson' `
